...
- `auth.ResourceLANIdentity` — the resource the LAN ops are gated on, separate
  from `"users"` because minting a login credential is not user administration
  (see Stage 6).
...
## Stage 4 — email-less users — **DONE, after unblocking a base-piece defect**

**Files:** `models.go`, `models_orm.go` (regenerate), `authority/users.go`.

> **Status: shipped.** The original text assumed `OmitEmpty: true` omits an
> empty email from the INSERT so the column stores NULL. Verified against the
> libraries as they were, that was false in **both** directions, and each half
> was a defect in a piece one layer down:
>
> 1. **Write.** `orm.DB.Create` writes every `Schema()` column unconditionally —
>    `model.Field.OmitEmpty` only guards `EncodeFields` (the wire codec). There
>    is no way to omit a column from an INSERT, so `""` is written and the
>    second email-less user collides on the unique index.
> 2. **Read.** Writing NULL (via a hand-built `storage.Query`) does insert
>    correctly, but reading the row back fails:
>    `sql: Scan error on column index 1, name "email": converting NULL to string is unsupported`.
>    `orm/qb.go` hands raw `*string` to `database/sql`. Every read path breaks
>    for that user — including `UserByID`, which `trusted_ip` calls, so the LAN
>    login of an email-less user fails.
>
> Per the harness (*"fix at the root, never at the leaf; a defect found in a
> base piece blocks this plan"*), neither was worked around here. The wave
> [`NULLABLE_COLUMNS_MASTER_PLAN.md`](https://github.com/webtyp/docs/blob/main/NULLABLE_COLUMNS_MASTER_PLAN.md)
> was written and **shipped**: `storage` (`NullSafe` — a NULL column scans as
> the Go zero value), `sqlite` and `postgres` (apply it at both scan seams), and
> `orm` (`Create` omits a zero-valued `OmitEmpty` column from the INSERT).
> This stage then completed against those tags.

What the stage landed as:

1. `models.go`, `UserModel` email field:
   ```go
   {Name: "email", Type: input.Email(), OmitEmpty: true, DB: &model.FieldDB{Unique: true}},
   ```
   Regenerated with `ormc`. With the `orm` fix shipped, this omits the column
   from the INSERT, so the database stores NULL — which a unique index allows to
   repeat.
2. `authority/users.go` `createUser`: no special case and no `db.RawConn()`. The
   unique-violation mapping to `ErrEmailTaken` stays for real emails; NULLs
   never collide.
3. `getUserByEmail` returns `ErrNotFound` for an empty email. This was **not**
   in the original plan and is a security fix found while writing the tests: an
   empty email is not a lookup key, and `oauth2`'s callback links the caller's
   provider identity to whatever `UserByEmail` returns — so a provider reporting
   a verified-but-empty address would have taken over the first account without
   one.
4. `tests/emailless_users_test.go` runs with no skip.
5. No schema migration needed: the column was already nullable (no `NotNull`).
...
3. `authority/ops.go` `MountOperations`, append:
   ...
   **Deviation from the original text (gate on `"users"`), decided during
   execution.** `register_lan` mints a LOGIN CREDENTIAL, not a user record:
   linking an attacker's RUT to the superadmin's `user_id` and then posting
   `PathLoginRUT` yields that user's session as soon as the caller's IP passes
   `TrustedIPStore`. Gating it on `"users"` would therefore make
   `users:create|update` imply account takeover wherever `TrustedIPStore`
   answers per-network instead of per-user — which is exactly what
   `mjosefa-cms` plans via `staff_manager` (see Out of scope). The two
   privileges are separated into `auth.ResourceLANIdentity` so an app grants
   LAN administration explicitly, and `unregister_lan` takes `model.Delete`
   (it removes the identity and its LANIP rows) rather than `model.Update`.
...
## Also in this branch — `tests/` became its own module

The suite runs against real SQLite (see `tests/go.mod` for why `storage/mem`
cannot stand in), which dragged `modernc.org/sqlite` and its driver tree into
the root module's graph even though no production package imports it —
`go list -deps ./...` found `webtyp.com/sqlite` zero times outside tests.

`tests/` now carries its own `go.mod` with `replace webtyp.com/auth => ..`, the
same pattern `webtyp.com/ormc/tests` already uses. The root module dropped
`sqlite`, `sqlt`, `server`, `unixid`, `form`, `dom` and `widget`, and **every**
third-party dependency: `modernc.org/*`, `github.com/*`, `golang.org/x/*`,
`howett.net/plist`. It is now 15 `webtyp.com/*` requires plus one indirect.

`gotest` already runs submodule directories that `./...` does not reach, so no
test stopped running. It did, at first, report 1.2 % coverage instead of the
real figure: the submodule ran with `-cover` but no `-coverprofile`, so its
result never reached the percentage. That was a defect in `webtyp/devflow`, not
here, and was fixed at the root — `gotest` now merges submodule profiles
(`devflow v0.4.92`) and this repo reports **64.4 %**.

## Also in this branch — `LIST_CONTRACT` phase C

Closing this branch required bringing the dependencies to their current tags,
which picked up `model v0.1.9` and `ormc v0.1.15` — phases A and B of
[`LIST_CONTRACT_MASTER_PLAN.md`](https://github.com/webtyp/docs/blob/main/LIST_CONTRACT_MASTER_PLAN.md).
`model.FielderSlice` no longer embeds `model.Fielder`, so a list stops claiming
it has columns. This repo's phase C therefore landed here rather than in a
separate plan:

- Regenerating `models_orm.go` dropped `Schema() nil` / `Pointers() nil` from
  all 13 list types; `Len`/`At`/`Append` are untouched.
- `tests/coverage_refactor_test.go` called `l.Schema()` / `l.Pointers()` on a
  `model.ModelSlice` in its smoke loop. Those two lines are gone — the methods
  no longer exist on the contract.

No `docs/PLAN_LIST_CONTRACT.md` is needed for this repo.

## Acceptance criteria

1. `go build ./...`, `go vet ./...`, `gotest ./...` green.
1b. `grep -rn "storage.Query" --include='*.go' .` → empty. A consumer
   hand-building a `storage.Query` would be the ORM leaking; stage 4 waited for
   the base fix instead of bypassing it. (`RawConn()` still appears in tests —
   `authority.Migrate(db.RawConn(), …)` is its documented signature, and a few
   fixtures force an expiry with it. That is wiring, not a bypass.)
1c. `grep -rn "List) Schema()\|List) Pointers()" --include='*.go' .` → empty.
